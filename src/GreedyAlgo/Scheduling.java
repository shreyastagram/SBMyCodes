...
public class Scheduling {
    ...
    public static void printBestSchedules(Schedule[] schedules){
        //use merge sort to sort schedules by ending times,
        //from earliest to last
        //Big(O)=O(1);
        Arrays.sort(schedules);

        //choose the first schedule form the sorted schedules since the first items in the sorted schedule array will always have the earliest ending time
        //Big(O)=O(1)
        ...
        for(int i=1;i<schedules.length;i++){
            //If the current schedule start time is later than the
            //last schedule ending time we can add this schedule
